...
## Fetch FASTQs (once)
You can populate `metadata/samplesheet.csv` and download FASTQs with:
...
```bash
nextflow run nf-core/fetchngs --input ./ids.csv --outdir /home/lab/kim/hd/rnaseq/reads_pe/raw/
```

Where `ids.csv` contains the `Experiment` values from `metadata/samples.tsv`. Point `params.samplesheet` in `nextflow.config` to the resulting `samplesheet.csv` (no need to copy it into this repo).
